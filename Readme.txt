...
アニメーション画像のサムネイル生成について：
 ・GIFアニメーション - ファイル
  imagickが利用できる場合、サムネイルもアニメーションで生成。
  利用できない場合は、静止画でサムネイルが生成されます。

　・アニメーションPNG(APNG) - ファイル
  imagick + ffmpegが利用できる場合は、アニメーションPNGとしてサムネイルも生成されます。
  利用できない場合は、1フレーム目な静止画のPNGファイルでサムネイルが生成されます。
...
Ver9.6 22/03/17 各種libの変更・調整。
Ver9.5 22/02/22 カテゴリな機能の追加。
Ver9.4 22/02/03 php8に対応。 imagickを利用出来る場合、apngのサムネイル生成に対応。
Ver9.3 20/01/17 imagickの使用が出来る場合、GIFアニメなサムネイルが可能に。
Ver9.2 19/12/01 Access処理の分離、日本以外からの投稿規制の追加。連続投稿規制をCookieからデータファイル方式に変更。
